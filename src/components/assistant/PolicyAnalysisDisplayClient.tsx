...
// Dynamic import with SSR disabled to prevent server-side pdfjs-dist import
const PdfViewerPane = dynamic(() => import('./PdfViewerPane'), { 
  ssr: false,
  loading: () => <div className="p-4 text-center text-gray-500">Loading PDF viewer...</div>
});
...
interface PolicyAnalysisDisplayClientProps {
...
export function PolicyAnalysisDisplayClient({ analysis, pdfUrl, fileName, rawAnalysisData, hideSave, hidePdfViewer }: PolicyAnalysisDisplayClientProps) {
  ...
  return (
    ...
    <div className="space-y-6">
      ...
      {/* PDF Viewer - Only render if not hidden and URL is available */}
      {!hidePdfViewer && (pdfUrl || safePdfUrl) && (
        <div className="bg-white dark:bg-gray-800 rounded-2xl p-4 border border-gray-200 dark:border-gray-700 shadow-sm">
          <div className="flex items-center gap-3 mb-3">
            <h4 className="font-semibold text-gray-900 dark:text-white">Vista del PDF</h4>
          </div>
          <PdfViewerPane 
            url={pdfUrl || safePdfUrl}
            height={400}
            labels={{ pdf: 'PDF', page: 'Página' }}
          />
        </div>
      )}

      {/* Save Policy Section - temporarily hidden by flag */}
      {ENABLE_SAVE_POLICY && !hideSave && (
