...
function loadInputFile(): { data: RawRecord[] } {
  const root = process.cwd();
  const candidateA = path.join(root, 'scripts', 'etl', 'raw', 'raw_webhound_sura.json');
  const candidateB = path.join(root, 'scripts', 'etl', 'raw_webhound_sura.json');
  const p = fs.existsSync(candidateA) ? candidateA : candidateB;
  if (!fs.existsSync(p)) {
    throw new Error(`Input file not found. Checked: ${candidateA} and ${candidateB}`);
  }
  const raw = fs.readFileSync(p, 'utf8');
  try {
    const parsed = JSON.parse(raw);
    if (Array.isArray(parsed?.data)) return { data: parsed.data as RawRecord[] };
    if (Array.isArray(parsed)) return { data: parsed as RawRecord[] };
...
function parseBenefitDetails(raw: any): Record<string, any> | null {
  ...
  const s0 = raw.trim();
  if (!s0) return null;
  // Heuristic: replace single quotes with double quotes if it looks like JSON-ish
  let s = s0;
  if (/\{\s*'/.test(s) || /':\s*'/.test(s)) {
    s = s.replace(/'/g, '"');
  }
  // Remove trailing commas in objects/arrays, if any
  s = s.replace(/,\s*([}\]])/g, '$1');
  try {
    return JSON.parse(s);
  } catch {
...
function isOfficialSuraUrl(u: string | null | undefined): boolean {
...
// Externalized synonyms loader
// Edit `scripts/etl/transform/synonyms.json` to add new provider-specific raw strings
// (e.g., MAPFRE, AXA, Allianz) under categories.{autos|salud}.{canonical_code}.
// No redeploy is required; this file is read at runtime.
// Example for MAPFRE: add "grua_mapfre_km" or similar to categories.autos.grua_km array.
type SynonymsConfig = {
  version?: number;
  updatedAt?: string;
...
const saludMap: Record<string, string> = {
  ...
  // variants observed in other sources
  atencion_domiciliaria: 'atencion_domiciliaria',
  // ——— New SURA → canonical mappings (accent/case tolerant) ———
  ...
  // maternidad
  ginecobstetra: 'maternidad',
  coberturas_congenitas_recien_nacido: 'maternidad',
  continuidad_atencion_parto: 'maternidad',
  // diagnosticos
  ayudas_diagnosticas: 'diagnosticos',
  examenes_laboratorio_clinico: 'diagnosticos',
  laboratorio_clinico_e_imagenologia: 'diagnosticos',
  ayudas_diagnosticas_imagenologia_hospitalarias_pac: 'diagnosticos',
  ayudas_diagnosticas_imagenologia_ambulatorias_pac: 'diagnosticos',
  // atencion_domiciliaria
  atencion_medica_domicilio: 'atencion_domiciliaria',
  // odontologia
  consulta_odontologia_general: 'odontologia',
  // medicamentos
  bolsa_medicamentos: 'medicamentos',
  medicamentos_post_atencion_medica: 'medicamentos',
  // telemedicina
  atencion_virtual_medico_general: 'telemedicina',
  atencion_virtual_medico_especialista: 'telemedicina',
  // salud_mental
  atencion_para_acompanamiento_emocional: 'salud_mental',
  // copago_bajo
  no_cobro_cuotas_moderadoras_pbs: 'copago_bajo',
  no_cobro_copagos_pbs: 'copago_bajo',
};
...
function normalizeBenefits(
...
): { codes: string[]; byCanonicalToRawKeys: Record<string, string[]> } {
  ...
  return { codes: Array.from(codesSet), byCanonicalToRawKeys: canonicalToRaw };
}
...
async function replacePlanBenefits(
  ...
  for (const code of canonicalCodes) {
    ...
    if (details && byCanonicalToRaw[code] && byCanonicalToRaw[code].length > 0) {
      // pick first matching raw key's details
...
async function main() {
  ...
  const { data } = loadInputFile();
  const inputPath = 'scripts/etl/raw/raw_webhound_sura.json (or fallback ./scripts/etl/raw_webhound_sura.json)';
  ...
  try {
    ...
    await withTx(pool, async (c) => {
      for (const rec of data) {
        ...
        // Gate: official SURA URL
        if (!isOfficialSuraUrl(flat.source_primary_url)) {
          report.rejected++;
        ...
        // Merge built-in map with external synonyms (external takes precedence on conflicts)
        ...
        const { codes: canonicalBenefits, byCanonicalToRawKeys } = normalizeBenefits(
          category,
          flat.benefits || [],
          mergedMap
        );
  ...
  const reportPath = path.join(outDir, `ingest_sura_plans_v3_report_${ts}.json`);
  const rejectsPath = path.join(outDir, `ingest_sura_plans_v3_rejected_${ts}.json`);
  fs.writeFileSync(reportPath, JSON.stringify(report, null, 2));
  fs.writeFileSync(rejectsPath, JSON.stringify(rejections, null, 2));

  console.log('[INGEST] SURA → plans_v3 complete');
  console.log(report);
}
