...
function loadInputFile(): { data: RawRecord[]; path: string } {
  ...
  const candidates = [
    path.join(root, 'scripts', 'etl', 'raw', 'raw_webhound_axacolpatria.json'),
    path.join(root, 'scripts', 'etl', 'raw_webhound_axacolpatria.json'),
  ];
...
function isOfficialAxaColpatriaUrl(u: string | null | undefined): boolean {
  if (!u || typeof u !== 'string' || !/^https?:\/\//i.test(u)) return false;
  try {
    const { hostname, pathname } = new URL(u);
    const host = hostname.toLowerCase();
    // Allow axacolpatria.co and subdomains
    if (host === 'axacolpatria.co' || host.endsWith('.axacolpatria.co')) return true;
    if (host === 'www.axacolpatria.co') return true;
    // Allow axa.com Colombian sections that reference Colombia
    if (host === 'axa.com' || host.endsWith('.axa.com')) {
...
// Externalized synonyms loader (shared pattern)
type SynonymsConfig = {
  version?: number;
  updatedAt?: string;
...
// Built-in defaults tuned for AXA COLPATRIA + shared canonical set
const saludMap: Record<string, string> = {
  ...
  hospitalizacion_domiciliaria: 'atencion_domiciliaria',
  optometria: 'consultas_especialistas',

  // AXA terms → canonical
  examenes_diagnostico: 'diagnosticos',
  reembolso_consultas: 'consultas_especialistas',
  reembolso_urgencias_exterior: 'asistencia_internacional',
  asistencia_internacional: 'asistencia_internacional',
  urgencias_alcohol: 'urgencias',
  urgencias_alcohol_sustancias: 'urgencias',
  cirugia_bariatrica: 'cirugias',
  tamizaje_embarazo: 'maternidad',

  // Allow these as-is (they will be ensured in benefits_master)
  terapias: 'terapias',
  vacunas: 'vacunas',
  trasplantes: 'trasplantes',
  cancer_tratamiento: 'cancer_tratamiento',
  vih_sida_tratamiento: 'vih_sida_tratamiento',
  enfermedades_congenitas: 'enfermedades_congenitas',
  amparo_desempleo: 'amparo_desempleo',
  continuidad_fallecimiento: 'continuidad_fallecimiento',
  deportes_alto_riesgo: 'deportes_alto_riesgo',
};
...
const autosMap: Record<string, string> = {
  ...
  proteccion_patrimonial: 'amparo_patrimonial',
  amparo_patrimonial: 'amparo_patrimonial',
  grua: 'grua_km',
  grua_km: 'grua_km',
  grua_transporte: 'grua_km',
  asistencia_vial: 'grua_km',
  perdida_total: 'perdida_total',
  'pérdida_total': 'perdida_total',
  perdidatotal: 'perdida_total',
  perdida_total_danos: 'perdida_total',
  'perdida_total_daños': 'perdida_total',
  perdida_total_hurto: 'perdida_total',
  danos_parciales: 'daños_parciales',
  'daños_parciales': 'daños_parciales',
  perdida_parcial_danos: 'daños_parciales',
  'perdida_parcial_daños': 'daños_parciales',
  perdida_parcial_hurto: 'daños_parciales',
  colision: 'colision',
  'colisión': 'colision',
  temblor_terremoto_erupcion_volcanica: 'eventos_catastroficos',
  ...
  // allow uncommon AXA extras as their own codes
  mensajes_urgentes: 'mensajes_urgentes',
  informe_vias: 'informe_vias',
  traslado_medico: 'traslado_medico',
};
...
async function main() {
  ...
  try {
    ...
    await withTx(pool, async (c) => {
      for (const rec of data) {
        ...
        // Gate: official AXA COLPATRIA URL
        if (!isOfficialAxaColpatriaUrl(flat.source_primary_url)) {
          report.rejected++;
  ...
  // Compute top unmapped terms across categories with rough suggestions (AXA flavor)
  const suggestions: Record<string, string> = {
    ...
    perdida_total_hurto: 'perdida_total',
    perdida_parcial_hurto: 'daños_parciales',
    informe_vias: 'asistencia_vial',
    traslado_medico: 'asistencia_en_viaje',
    mensajes_urgentes: 'mensajes_urgentes',
    reembolso_consultas: 'consultas_especialistas',
    reembolso_urgencias_exterior: 'asistencia_internacional',
    cirugia_bariatrica: 'cirugias',
  ...
  const reportPath = path.join(outDir, `ingest_axacolpatria_plans_v3_report_${ts}.json`);
  const rejectsPath = path.join(outDir, `ingest_axacolpatria_plans_v3_rejected_${ts}.json`);
  fs.writeFileSync(reportPath, JSON.stringify(report, null, 2));
  fs.writeFileSync(rejectsPath, JSON.stringify(rejections, null, 2));

  console.log('[INGEST] AXA COLPATRIA → plans_v3 complete');
  console.log(report);
}
