...
function loadInputFile(): { data: RawRecord[]; path: string } {
  ...
  const candidates = [
    path.join(root, 'scripts', 'etl', 'raw', 'raw_webhound_hdi.json'),
    path.join(root, 'scripts', 'etl', 'raw_webhound_hdi.json'),
    path.join(root, 'scripts', 'etl', 'raw_webhound_HDI.json'),
  ];
...
function isOfficialHdiUrl(u: string | null | undefined): boolean {
  ...
  try {
    ...
    const allowed = [
      'hdiseguros.com.co',
      'www.hdiseguros.com.co',
    ];
    if (allowed.includes(host)) return true;
    return host.endsWith('.hdiseguros.com.co');
  } catch {
    return false;
  }
}
...
// Built-in defaults + HDI-specific enrichments (for sentence-style benefits)
...
const autosMap: Record<string, string> = {
  ...
  asistencia_exequial: 'asistencia_exequial',
  grua: 'grua_km',
  grua_km: 'grua_km',
...
function normalizeBenefits(
  ...
  function heuristicsForSentence(cat: string, sentenceRaw: string): string[] {
    ...
    if (cat === 'autos') {
      ...
      if (/lucro\s+cesante|salario\s+diario/.test(s)) found.push('lucro_cesante');
      if (/exequial/.test(s)) found.push('asistencia_exequial');
      ...
      if (/terremoto|temblor|erupcion\s+volcanica/.test(s)) found.push('eventos_catastroficos');
    } else if (cat === 'salud') {
      if (/hospitalizacion/.test(s)) found.push('hospitalizacion');
      if (/cirugia/.test(s)) found.push('cirugias');
      if (/maternidad/.test(s)) found.push('maternidad');
      if (/odontolog/.test(s)) found.push('odontologia');
      if (/urgenc/.test(s)) found.push('urgencias');
      if (/laboratorio|imagenolog|diagnostico/.test(s)) found.push('diagnosticos');
      if (/consulta(s)?\s+pre\-?hospitalari/.test(s)) found.push('consultas_especialistas');
      if (/psiquiatr|psicolog/.test(s)) found.push('salud_mental');
      if (/domiciliaria?/.test(s)) found.push('atencion_domiciliaria');
      if (/telemedicina|virtual/.test(s)) found.push('telemedicina');
      if (/alto\s+costo|catastrof/.test(s)) found.push('enfermedades_alto_costo');
      if (/segunda\s+opinion/.test(s)) found.push('segunda_opinion');
      if (/viaje/.test(s)) found.push('asistencia_en_viaje');
    }
  ...
  for (const b of raw || []) {
    ...
    // HDI: sentence-style benefits → derive canonical codes via heuristics
    const derived = heuristicsForSentence(category, k);
    if (derived.length > 0) {
...
async function main() {
  ...
  try {
    ...
    await withTx(pool, async (c) => {
      for (const rec of data) {
        ...
        // Gate: official HDI URL
        if (!isOfficialHdiUrl(flat.source_primary_url)) {
          report.rejected++;
  ...
  // Compute top unmapped terms across categories with rough suggestions (HDI flavor)
  const suggestions: Record<string, string> = {
    ...
    asistencia_juridica: 'asistencia_juridica',
    accidentes_personales: 'accidentes_personales',
    salario_diario: 'lucro_cesante',
    lucro_cesante: 'lucro_cesante',
    terremoto: 'eventos_catastroficos',
    temblor: 'eventos_catastroficos',
    erupcion_volcanica: 'eventos_catastroficos',
    segunda_opinion: 'segunda_opinion',
    alto_costo: 'enfermedades_alto_costo',
  };
  ...
  const reportPath = path.join(outDir, `ingest_hdi_plans_v3_report_${ts}.json`);
  const rejectsPath = path.join(outDir, `ingest_hdi_plans_v3_rejected_${ts}.json`);
  fs.writeFileSync(reportPath, JSON.stringify(report, null, 2));
  fs.writeFileSync(rejectsPath, JSON.stringify(rejections, null, 2));

  console.log('[INGEST] HDI → plans_v3 complete');
  console.log(report);
}
