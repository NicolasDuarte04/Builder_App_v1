...
function loadInputFile(): { data: RawRecord[]; path: string } {
  ...
  const candidates = [
    path.join(root, 'scripts', 'etl', 'raw', 'raw_webhound_allianz.json'),
    path.join(root, 'scripts', 'etl', 'raw_webhound_allianz.json'),
  ];
...
function isOfficialAllianzUrl(u: string | null | undefined): boolean {
  if (!u || typeof u !== 'string' || !/^https?:\/\//i.test(u)) return false;
  try {
    const { hostname, pathname } = new URL(u);
    const host = hostname.toLowerCase();
    // Allow allianz.co and subdomains
    if (host === 'allianz.co' || host.endsWith('.allianz.co')) return true;
    // Allow allianz.com Colombian sections
    if (host === 'allianz.com' || host.endsWith('.allianz.com')) {
      const p = pathname.toLowerCase();
      if (/(^|\/)co(\/|$)/.test(p) || /colombia/.test(p)) return true;
    }
    // Allow iberolatam assets under allianz-co
    if (host.includes('allianz-co')) return true;
    return false;
  } catch {
    return false;
  }
...
const saludMap: Record<string, string> = {
  ...
  red_prestadores_top: 'consultas_especialistas',
...
async function main() {
  ...
  try {
    ...
    await withTx(pool, async (c) => {
      for (const rec of data) {
        ...
        // Gate: official Allianz URL
        if (!isOfficialAllianzUrl(flat.source_primary_url)) {
          report.rejected++;
  ...
  const suggestions: Record<string, string> = {
    asistencia_vial: 'grua_km',
    conductor_elegido: 'conductor_viaje',
    cobertura_total: 'perdida_total',
    gastos_movilizacion: 'gastos_movilizacion',
    accidentes_personales: 'accidentes_personales',
    repatriacion_medica: 'repatriacion_medica',
    urgencia_odontologica: 'odontologia',
    telemedicina: 'telemedicina',
  };
  ...
  const reportPath = path.join(outDir, `ingest_allianz_plans_v3_report_${ts}.json`);
  const rejectsPath = path.join(outDir, `ingest_allianz_plans_v3_rejected_${ts}.json`);
  fs.writeFileSync(reportPath, JSON.stringify(report, null, 2));
  fs.writeFileSync(rejectsPath, JSON.stringify(rejections, null, 2));

  console.log('[INGEST] Allianz → plans_v3 complete');
  console.log(report);
}
