...
function loadInputFile(): { data: RawRecord[]; path: string } {
  ...
  const candidates = [
    path.join(root, 'scripts', 'etl', 'raw', 'raw_webhound_seguros_bolivar.json'),
    path.join(root, 'scripts', 'etl', 'raw_webhound_seguros_bolivar.json'),
  ];
...
function isOfficialBolivarUrl(u: string | null | undefined): boolean {
  if (!u || typeof u !== 'string' || !/^https?:\/\//i.test(u)) return false;
  try {
    const { hostname } = new URL(u);
    const host = hostname.toLowerCase();
    if (host === 'segurosbolivar.com' || host.endsWith('.segurosbolivar.com')) return true;
    if (host === 'grupobolivar.com.co' || host.endsWith('.grupobolivar.com.co')) return true;
    return false;
  } catch {
    return false;
  }
...
// Externalized synonyms loader (shared)
type SynonymsConfig = {
  version?: number;
  updatedAt?: string;
...
// Defaults (merge with external synonyms). Include broad Salud/Autos and some English tokens seen in Bolívar PDFs.
const saludMap: Record<string, string> = {
  ...
  consultas_especialistas: 'consultas_especialistas',
...
function normalizeBenefits(
  ...
  function addCode(code: string, rawKey: string) {
    codesSet.add(code);
    if (!canonicalToRaw[code]) canonicalToRaw[code] = [];
    canonicalToRaw[code].push(rawKey);
  }

  function heuristics(cat: string, sentenceRaw: string): string[] {
    const s = stripAccents(String(sentenceRaw || '')).trim().toLowerCase();
    const found: string[] = [];
    if (cat === 'autos') {
      if (/(responsabilidad\s*civil|danos\s+que\s+puedas\s+ocasionar\s+a\s+otras\s+personas)/.test(s)) found.push('rc');
      if (/(perdida\s+total|perdidas\s+totales)/.test(s)) found.push('perdida_total');
      if (/(perdidas\s+parciales|danos\s+parciales)/.test(s)) found.push('daños_parciales');
      if (/asistencia\s+juridica/.test(s)) found.push('asistencia_juridica');
      if (/amparo\s+patrimonial/.test(s)) found.push('amparo_patrimonial');
      if (/accidentes\s+personales/.test(s)) found.push('accidentes_personales');
      if (/vidrio/.test(s)) found.push('cobertura_vidrios');
      if (/llanta/.test(s)) found.push('cobertura_llantas');
      if (/carro\s+reemplazo|vehiculo\s+sustituto/.test(s)) found.push('carro_reemplazo');
      if (/grua|asistencia\s+vial/.test(s)) found.push('grua_km');
    } else if (cat === 'salud') {
      if (/hospitalizacion|hospitalization|hospitalisation/.test(s)) found.push('hospitalizacion');
      if (/cirugia/.test(s)) found.push('cirugias');
      if (/maternidad|newborn/.test(s)) found.push('maternidad');
      if (/odontolog/.test(s)) found.push('odontologia');
      if (/urgenc|emergency/.test(s)) found.push('urgencias');
      if (/laboratorio|imagenolog|diagnostico|outpatient/.test(s)) found.push('diagnosticos');
      if (/consulta(s)?\s+pre\-?hospitalari|consultas?\s+especialistas?/.test(s)) found.push('consultas_especialistas');
      if (/psiquiatr|psicolog|psychiatr/.test(s)) found.push('salud_mental');
      if (/domiciliari(a|o)|home\s+hospital/.test(s)) found.push('atencion_domiciliaria');
      if (/telemedicina|virtual|telefonic/.test(s)) found.push('telemedicina');
      if (/segunda\s+opinion|second\s+opinion/.test(s)) found.push('segunda_opinion');
      if (/exterior|abroad/.test(s)) found.push('asistencia_en_viaje');
    }
  ...
  for (const b of raw || []) {
    ...
    const derived = heuristics(category, k);
    if (derived.length > 0) {
      for (const code of derived) addCode(code, k);
      continue;
    }
    unmapped.push(k);
  }
...
function canonicalizePricingModel(pm: string | null, priceMin: number | null): 'quote' | 'monthly' | 'annual' | null {
  const p = String(pm || '').toLowerCase();
  if (p === 'quote') return 'quote';
  if (p === 'monthly') return 'monthly';
  if (p === 'annual') return 'annual';
  if (p === 'direct' || p === 'directo') return priceMin != null ? 'annual' : null;
  return null;
}
...
async function main() {
  ...
  try {
    ...
    await withTx(pool, async (c) => {
      for (const rec of data) {
        ...
        if (!flat) {
          report.rejected++;
          rejections.push({ reason: 'missing_required_core_fields' });
        ...
        // Gate: official Seguros Bolívar / Grupo Bolívar URL
        if (!isOfficialBolivarUrl(flat.source_primary_url)) {
          report.rejected++;
        ...
        const planId = await upsertPlan(c, {
          ...
          pricing_model: canonicalizePricingModel(flat.pricing_model, flat.price_min),
  ...
  // Compute top unmapped terms with suggestions for Bolívar flavor
  const suggestions: Record<string, string> = {
    'orientacion medica telefonica': 'telemedicina',
    'asistencia en caso de urgencia en el exterior': 'asistencia_en_viaje',
    'asistencia en el exterior': 'asistencia_en_viaje',
    'emergency abroad': 'asistencia_en_viaje',
    'second opinion': 'segunda_opinion',
    'segunda opinion': 'segunda_opinion',
    'médico en casa 24/7': 'atencion_domiciliaria',
    'medico en casa 24/7': 'atencion_domiciliaria',
    'examenes e imagenes diagnosticas': 'diagnosticos',
    'examenes diagnosticos': 'diagnosticos',
  ...
  const reportPath = path.join(outDir, `ingest_seguros_bolivar_plans_v3_report_${ts}.json`);
  const rejectsPath = path.join(outDir, `ingest_seguros_bolivar_plans_v3_rejected_${ts}.json`);
  fs.writeFileSync(reportPath, JSON.stringify(report, null, 2));
  fs.writeFileSync(rejectsPath, JSON.stringify(rejections, null, 2));

  console.log('[INGEST] Seguros Bolívar → plans_v3 complete');
  console.log(report);
}
